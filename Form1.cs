...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private void ExecutaAtualizacao(String att, String agora)
        {
            ...
            using (var con = new MySql.Data.MySqlClient.MySqlConnection("Server=192.168.100.62;Database=tng_pas_mod;Uid=root;Pwd=tng@123;"))
            {
                con.Open();
                ...
                using (var cmd = new MySql.Data.MySqlClient.MySqlCommand("SELECT idVeiculoSascar, placa FROM veic_aux WHERE idVeiculoSascar >1", con))
                {
                    ...
                    for (int i = 0; i < idVeiculosParaAtualizar.Count; i++)
                    {
                        ...
                        //PONTO DE RETORNO APÓS AGUARDAR OS SEGUNDOS DETERMINADOS PARA REFAZER A PESQUISA COM O MESMO IDVEICULO.
                        //pesquisa:

                        var pacotePosicao = servico.obterPacotePosicaoHistorico("INTEGRACOOPER", "sascar1", att, agora, veiculo.IdVeiculo, true);
...

                        {
                            ...
                            using (var connection = new MySql.Data.MySqlClient.MySqlConnection("Server=192.168.100.62;Database=tng_pas_mod;Uid=root;Pwd=tng@123;"))
                            {
                                connection.Open();
        ...
        {
            //lblTimerPos.Text = "0" + horaPos + ":" + minutoPos + ":" + segundoPos;
                ...
                else
                {
                    using (SmtpClient smtp = new SmtpClient())
                    {
                        using (MailMessage email = new MailMessage())
                        {
                            ...
                            smtp.Credentials = new System.Net.NetworkCredential("integracao.coopercargo@gmail.com", "coopercargo@2020");
                            smtp.Port = 587;
                            smtp.EnableSsl = true;

                            //Email (Mensagem)
                            email.From = new MailAddress("integracao.coopercargo@gmail.com");
                            email.To.Add("gilvanefreire@gmail.com");

                            email.Subject = "Erro no Integrador";
                            email.IsBodyHtml = true;
                            email.Body = "<h1>Atenção</h1><br><p>Olá gestor, houve alguma falha de comunicação entre Sascar - Coopercargo, favor reiniciar o aplicativo</p>";
    ...
    private void AtualizaVeic()
    {
        ...
        var pacoteVeiculos = servico.obterVeiculos("INTEGRACOOPER", "sascar1", 1000, true, 1, false);
    ...
    private void AtualizaCliente()
    {
        ...
        try
        {
            ...
            foreach (var clt in listClientes)
            {
                ...
                //chave gil
                //string requestUri = string.Format("https://maps.googleapis.com/maps/api/geocode/xml?key={1}&address={0}&sensor=false", Uri.EscapeDataString(address), "AIzaSyBGPPNmhFjorI0TCTEQKdHGunq_qp3RYDg");
                string requestUri = string.Format("https://maps.googleapis.com/maps/api/geocode/xml?key={1}&address={0}&sensor=false", Uri.EscapeDataString(address), "AIzaSyCBgCrLL3zwB-BkGqGtugo90880PxHtb78");
    ...
    private void SalvaLogErro()
    {
        ...
        using (ExcelPackage excel = new ExcelPackage())
        {
            ...
            Directory.CreateDirectory(@"C:\Log_Integrador_Coopercargo");

            FileInfo excelFile = new FileInfo(@"C:\Log_Integrador_Coopercargo\teste.xlsx");
